...
구현 기록:
- 대기 차량 상세의 보험/일반/장기 버튼을 `배차` 단일 버튼으로 통합
- 배차 입력창 상단에 보험/일반/장기 선택 UI 추가
- 선택 유형별 기존 `status`/`status_action` 값(`배차 보험`, `배차 일반`, `배차 장기`) 유지
- 외부/실내 세차 버튼을 `세차` 단일 버튼으로 통합하고 선택창에서 외부세차/실내세차 실행
- 주차 다이얼로그는 최초 표시 시 기본 주차 목록만 노출하고 `직접추가` 클릭 시 새 주차지 입력폼 표시
