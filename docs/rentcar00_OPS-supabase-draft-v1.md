...
판단:
- MVP에서는 복잡한 내부 `status_key` 보다 `tab_key` 중심이 더 안전하다.
- 원본 시트 상태는 `rc00_ops_reservations.status_raw` 로 유지한다.
- 카드용 원장 필드와 탭 계산 필드를 분리하면 sync 충돌과 상태 변경 책임이 깔끔해진다.
...
- 시트 `예약상태` 원문은 `status_raw` 로 유지하고, 앱은 `tab_key` 를 계산한다

이유:
- 기존 AppSheet와 데이터 의미를 맞추기 쉽다
- 원본 상태와 앱 표시 상태의 책임을 분리할 수 있다
