...
- 신규 active 개발은 없고, 실기기/운영 확인과 다음 회계 프로그램 기획만 남긴 상태다.
...
- 최근 운영 DB 정리 내역을 문서화했다.
  ...
  - 신규 차량 `165허8095` 모닝, `175호2135` 스타리아를 OPS DB에 추가했다.
  - 두 차량 초기 상태는 `대기중`으로 정리했다.
...
2. 홈페이지 차량 추가/동기화는 OPS repo에 아직 없으므로 홈페이지 쪽 구조 확인이 필요하다.
3. 다음 회계 프로그램은 OPS 예약/차량 데이터와 연결되는 별도 설계가 필요하다.
...
2. 지난 미배차와 3일 이내 미배차가 `배차대기`에 남고, D+1/D+2/D+3/오늘 배차 배지가 구분되는지 확인한다.
...
### 1차 장애 확인 포인트
1. 실기기에서 `배차대기` 탭에 과거 미배차/오늘 배차/3일 이내 배차 임박이 의도대로 보이는지 확인한다.
...
- 배차일이 지났거나 오늘부터 3일 이내이고, 배차 일정이 미완료이며 예약상태가 아직 `배차중/완료`가 아니면 `배차대기`에 남는다.
- 카드 배지는 `배차 지연`, `오늘 배차`, `배차일+1`, `배차일+2`, `배차일+3`, `반납 지연`, `오늘 반납`으로 상태를 구분한다.
