...
- 자동차/차종 그룹 기준을 먼저 잠근 뒤 그룹별 가격 정책을 재설정한다.
- 가격 정책 수정은 예약 생성/수정, IMS import, 홈페이지 이벤트, 정산 후보 영역과 연결될 수 있으므로 좁은 phase로 나눈다.
- 과거 홈페이지 가격 조정 문서는 현재 active 기준으로 확인되지 않았다. 관련 archive 문서는 참고용으로만 보고, 현재 목표는 이 문서를 기준으로 다시 고정한다.
...
### 1순위: 자동차 그룹별 가격 정책 재설정
- 자동차/차종 그룹 기준 확정
- 그룹별 가격 정책 입력/저장 위치 확인
- 예약 직접 생성/수정 경로 가격 적용 확인
- IMS import 가격 매핑 확인
