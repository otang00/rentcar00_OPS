...
- 예약별 상태는 예약 1건당 1행이며 MVP 기준 `tab_key`, 현재 체크값을 가진다.
...
1. 시트 `예약상태='예약중'` 이면 `예약중`
2. 그중 `대여일`이 오늘이면 `오늘배차`
3. 시트 `예약상태='배차중'` 이면 `배차중`
4. 그중 `반납일`이 오늘이면 `반납일`
5. 시트 `예약상태='반납완료'` 이면 `완료/기록`
6. 시트 `예약상태='예약취소'` 는 기본 탭 제외

즉, 운영자는 “현재 해야 할 버튼”만 보고 누르면 되고,
앱은 그 결과를 기록하되 MVP 기준 탭 판정은 시트 `예약상태` 원문을 우선 따라간다.
...
### 고정 보조 규칙
- 오늘배차는 준비 미완료여도 진입을 막지 않는다.
- 배차중 진입 기준은 별도 출발 체크가 아니라 시트 `예약상태='배차중'` 변경이다.
- 반납일 진입 기준은 `예약상태='배차중'` 유지 상태에서 `end_at` 이 오늘인 경우다.
- 완료 진입 기준은 시트 `예약상태='반납완료'` 다.

### 우선순위 원칙
1. 시트 원문 `status_raw`
2. `start_at` / `end_at` 날짜 계산
3. 앱 내부 `tab_key`
4. 체크/경고 보조값

### 충돌 처리 원칙
- MVP에서는 복잡한 수동 상태보다 시트 `예약상태` 원문을 우선한다.
- 앱 내부 상태는 탭 계산 보조와 체크/경고 표시에 집중한다.
...
색상/강조 규칙:
- 1줄 우측 일시에서 날짜는 강조, 시간은 더 강하게 강조한다.
- 1줄 좌측 이름 영역은 고객명이 아니라 차량명으로 고정한다.
